...
PetscErrorCode ComputeRHS(KSP ksp, Vec b, void *ctx) {
  ...
  // info.xs, info.ys: starting indices of local portion
  // info.xm, info.ym: local sizes in x and y
  // info.mx, info.my: global grid sizes
  // 
  // All nx*ny points are interior unknowns (matching Python interpretation)
  // Boundaries are handled implicitly in the matrix stencil
  for (j = info.ys; j < info.ys + info.ym; j++) {
    for (i = info.xs; i < info.xs + info.xm; i++) {
      // All points are interior: RHS = f = c (constant source term)
      array[j][i] = user->c;
    }
  }
...
PetscErrorCode ComputeMatrix(KSP ksp, Mat A, Mat P, void *ctx) {
  ...
  // Retrieve the distributed array and grid information//! <DEBUG >Get the MPI rank for debug printing (remove this code for full test)
  //PetscMPIInt   rank;
  ...
  for (j = info.ys; j < info.ys + info.ym; j++) {
    for (i = info.xs; i < info.xs + info.xm; i++) {
      ...
      // Center: S_{i,j} - always included with full diagonal coefficient
      col[ncols].i = i;
      col[ncols].j = j;
      v[ncols]     = 2.0 / hx2 + 2.0 / hy2;
      ncols++;

      // Bottom neighbor: S_{i,j-1} - only if j > 0
      if (j > 0) {
        col[ncols].i = i;
        col[ncols].j = j - 1;
        v[ncols]     = -1.0 / hy2;
      ...
      // Top neighbor: S_{i,j+1} - only if j < my-1
      if (j < info.my - 1) {
        col[ncols].i = i;
        col[ncols].j = j + 1;
        v[ncols]     = -1.0 / hy2;
      ...
      // Left neighbor: S_{i-1,j} - only if i > 0
      if (i > 0) {
        col[ncols].i = i - 1;
        col[ncols].j = j;
        v[ncols]     = -1.0 / hx2;
      ...
      // Right neighbor: S_{i+1,j} - only if i < mx-1
      if (i < info.mx - 1) {
        col[ncols].i = i + 1;
        col[ncols].j = j;
        v[ncols]     = -1.0 / hx2;
